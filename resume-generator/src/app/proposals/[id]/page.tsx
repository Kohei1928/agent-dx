...
export default function ProposalDetailPage() {
  ...
  const getShareUrl = () => {
    if (typeof window !== "undefined" && proposal) {
      return `${window.location.origin}/proposal/${proposal.shareToken}`;
    }
    return "";
  };
        ...
        <div className="card p-6 mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-6">
              ...
              {proposal.status === "sent" && proposal.viewCount > 0 && (
                <div>
                  <p className="text-sm text-slate-500">閲覧回数</p>
                  <p className="font-medium text-blue-600">{proposal.viewCount}回</p>
                </div>
              )}
            </div>
