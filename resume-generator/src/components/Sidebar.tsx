...
function SettingsAccordion({ 
  ...
  if (isCollapsed) {
    return (
      <Link
        href="/settings/generation-templates"
        className={`flex items-center justify-center px-2 py-1.5 rounded-lg transition-all text-xs ${
          isAnyActive
  ...
  return (
    <div>
      <button
        ...
        className={`w-full flex items-center justify-between px-3 py-1.5 rounded-lg transition-all text-xs ${
          isAnyActive && !isOpen
            ? "bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-md shadow-orange-200"
            : "text-slate-500 hover:bg-orange-50 hover:text-orange-600"
        }`}
          ...
          {items.map((item) => {
            ...
            return (
              <li key={item.href}>
                ...
                >
                  <span className="flex-shrink-0 [&>svg]:w-3 [&>svg]:h-3">{item.icon}</span>
      ...
      <div className={`${isCollapsed ? "p-3" : "p-4"} border-b border-slate-100`}>
        {isCollapsed ? (
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <button
              onClick={onToggle}
              className="w-10 h-8 flex items-center justify-center bg-gradient-to-r from-orange-500 to-orange-600 text-white hover:from-orange-600 hover:to-orange-700 rounded-lg shadow-md shadow-orange-200 transition-all"
              title="サイドバーを展開"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
